...
// establish correct look up for CH4 and N2O factors
var fuelType
if (type == "wood and wood waste") {
  fuelType = "wood";
} else {
  fuelType = "pulping liquors";
  burningContext = "industry" // always default to 'industry' - the only option
}

// get CH4 emissions factor
massCH4PerEnergy = parseFloat(dataFinder.getDataItemValue('business/energy/stationaryCombustion/epa/ch4andn2ofactors', 'type='+fuelType+',context='+burningContext, 'massCH4PerEnergy'));
massCH4PerEnergy /= 1000.0; // convert g to kg
 
// get N2O emissions factor
massN2OPerEnergy = parseFloat(dataFinder.getDataItemValue('business/energy/stationaryCombustion/epa/ch4andn2ofactors', 'type='+fuelType+',context='+burningContext, 'massN2OPerEnergy'));
