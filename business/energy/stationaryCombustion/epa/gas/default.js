...
var quantity
if (volume == null && energyConsumed == null) {
    quantity = 0;
  returnValues.addNote('comment', 'No calculation made. Please specify an energy or volume quantity');
} else if (energyConsumed == null) {
  quantity = volume * (energyPerVolume / 1000000);
  returnValues.addNote('comment', 'Calculation made on the basis of a volume quantity');
} else if (volume == null) {
...
massCH4PerEnergy = parseFloat(dataFinder.getDataItemValue('business/energy/stationaryCombustion/epa/ch4andn2ofactors', 'type=natural gas,context='+burningContext, 'massCH4PerEnergy'));
massCH4PerEnergy /= 1000.0; // convert g to kg
 
// get N2O emissions factor
massN2OPerEnergy = parseFloat(dataFinder.getDataItemValue('business/energy/stationaryCombustion/epa/ch4andn2ofactors', 'type=natural gas,context='+burningContext, 'massN2OPerEnergy'));
