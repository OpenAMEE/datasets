...
// determine quantity type specified in profile item
var quantity
try{
  quantity = livestockNumber * typicalMass;
} 
catch(err) {
  quantity = totalLiveweight;
}
...
if (fractionVolatilised == null) { // direct only calculation
  n2oEmissions = quantity * (massNExcretedPer1000kgLivestockPerTime * 365 / 1000) * massN2OPerMassNDirect * stoich;
} else { // direct plus indirect emissions calculation
  n2oEmissions = quantity * (massNExcretedPer1000kgLivestockPerTime * 365 / 1000) * (massN2OPerMassNDirect + (fractionVolatilised * massN2OPerMassNVolatilised)) * stoich;
}
