...
if (typeof(offsiteSinterQuantity) == 'undefined') {
  setCO2ToZero();
  missingRequiredInputNote('CO2','offsiteSinterQuantity');
}

if (typeof(carbonSinter) == 'undefined') {
  setCO2ToZero();
  missingRequiredInputNote('CO2','carbonSinter');
}
...
if (co2Calculable == true) {
  co2Emissions = ((materialQuantity * carbonContent) - (offsiteSinterQuantity * carbonSinter)) * parseFloat(stoich);
}

if (typeof(sinterQuantity) == 'undefined') {
  ch4Emissions = 0;
  // Declare that CO2e cannot be calculated
  ch4CO2eCalculable = false;
  missingRequiredInputNote('CH4','sinterQuantity');
} else {
  ch4Emissions = sinterQuantity * ch4EM;
