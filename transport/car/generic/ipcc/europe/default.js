...
if(fuel == "gasoline"){
...
} else if(fuel == "diesel") {// i.e. diesel
     ...
     CO2VolumeFactor = (massCO2PerEnergy * energyPerVolume) / 1000;
} else if(fuel == "cng") {// i.e. diesel
     var perGallonToPerLitre = 0.2641720523;
     CO2VolumeFactor = dataFinder.getDataItemValue('transport/byFuel/us','fuel=cng,CO2eOrCO2=CO2 only','kgCO2PerUsGallon');
...
if(distance == "" && distancePerJourney == ""){
     ...
     if (distancePerJourney == ""){
          if(fuelEfficiency == ""){ //CO2 fuel-based calcualtion
               ((distance * nonCO2DistanceFactor) + (CO2VolumeFactor * fuelConsumed))/occupancy;
          } else { //CO2 distance-based calculation
               CO2DistanceFactor = CO2VolumeFactor / fuelEfficiency;
               (distance * (nonCO2DistanceFactor + CO2DistanceFactor))/occupancy;
          }
     } else {
          ...
          if(fuelEfficiency == ""){ //CO2 fuel-based calcualtion
               ((distancePerJourney * journeys * nonCO2DistanceFactor) + (CO2VolumeFactor * fuelConsumed))/occupancy;
          } else { //CO2 distance-based calculation
               CO2DistanceFactor = CO2VolumeFactor / fuelEfficiency;
               (distancePerJourney * journeys * (nonCO2DistanceFactor + CO2DistanceFactor))/occupancy;
          }
     }
}
